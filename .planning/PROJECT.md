...
- [x] Answer requests are served from a priority queue (manual Ctrl+Alt+A preempts queued auto-answers; nothing cancels an in-flight stream; bursts are debounced + run single-in-flight so they never spawn parallel Claude calls) — Validated in Phase 10: Priority Answer Queue (AA-05/AA-06; 10/10 must-haves incl. the D-11 late-delta-bleed invariant closed via a gap-closure pass; 207/207 tests, 2026-07-07). The enabling backend for v1.2 auto-answers.
...
---
*Last updated: 2026-07-07 — Milestone v1.1 (Structured Q/A Panel) shipped, archived, and tagged v1.1.0. Started milestone v1.2 (Auto-Answer for Detected Questions): auto-generate an AI answer when the Q/A panel identifies a question, streamed into the existing AI panel, with a 3-state scope hotkey (All → Directed-at-me → Off), a local no-AI directed-at-me heuristic, and a priority answer queue (manual preempts) replacing the single-in-flight drop-if-busy behavior. Deliberately reverses the v1 "AI calls user-triggered only" constraint for answer generation; detection stays local. Phase 10 (Priority Answer Queue) complete — the queue backend (AA-05/AA-06) is in place with the D-11 cross-request-bleed invariant closed; Phases 11 (Auto-Answer Trigger) and 12 (Scope Hotkey + Directed-at-Me) remain.*
